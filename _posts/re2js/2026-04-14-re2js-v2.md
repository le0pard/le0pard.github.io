...
To achieve full PCRE compliance, a regex engine needs to support Unicode property escapes (e.g., `\p{Greek}`, `\p{Emoji}`). However, shipping the raw code-point lookup arrays for the entire Unicode specification usually results in megabytes of bundle bloat.
